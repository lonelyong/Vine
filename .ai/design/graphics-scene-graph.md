...
**变更行为钉子（2026-09-25，`SceneMutationTest.*`，4 例）**：四种用户变更各自的整链后果——
换父（结构单父 + 命令摆放 + **新旧两条链都重算** + 继承整体切换：材质/程序/不透明度按新链解析；成环拒绝另见 `NodeTest.AddChildRejectsCycles`）、
换材质/换着色器（下**一**次收集带新身份，无缓存层；清空回继承）、移除（命令消失 + 旧链重算 + 重加回仍可画）。
**一个修正过的判断**：漏公告的后果**不对称**——漏"入库侧"（新父 `addChild`）才是物体消失（陈旧空盒把内容剔掉）；
漏"出库侧"（旧父 `removeChild`）只是陈旧**超集**盒（多走一层空子树，性能级，画面仍对）⇒ 抓出库侧必须用
`boundsRecomputeCount()` 证人（`scene.boundingBox()` 是新鲜走线，照不出漏公告）。三条变异（两侧静默 + 换父不摘旧父）全部红。
