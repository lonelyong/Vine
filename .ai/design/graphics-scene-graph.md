...
每个节点持久保存**世界盒** + 一个**有效性判据**；子节点变化时上行并集更新。收益与代价：

- **静态内容 + 相机在动**（最常见的查看/环绕）：**全额收益**——世界盒与相机无关，没有东西需要更新（今天却每帧重算 O(N)）。
- **内容也在动**：更新成本 ∝ 改动子树 × 深度 ⇒ 赢的条件是"**存在但不可见的节点数 ≫ 每帧被改动的节点数**"（大世界 / LOD / 大装配）。
- **内存**：`Aabbd` = 6 double = **48 B/节点**（100k 节点 ≈ 4.8 MB）。
- **风险（要靠契约解决，不能靠"记得通知"）**：
  - `Node::localTransformMatrix()` / `boundingBox()` 都是**虚函数** ⇒ 宿主自定义节点可以自己合成变换与盒，外部无从得知
    ⇒ 任何"每条变更路径各自调用失效"的方案都**不 sound**；
  - 漏一次失效的后果是**剔错 ⇒ 物体凭空消失**（不是慢一帧），属于"所有结构门禁都绿"也照样漏出去的一类
    （本仓库先例：V5 那两个 define 分支**从未编译过**，而全部门禁是绿的）；
  - 所以形状必须是**版本键校验的保守缓存**，并给自定义节点一个**显式失效口**。本仓库**已有**这个契约先例
    （`Geometry::setRevision()`、`Scene::invalidateContent()` 都是"变更由调用者 announce"）：**要么**把
    "变换/数据变更必须 announce"写进 SDK 契约（自定义节点随之受限），**要么**接受"自定义节点下只能每次从零推"。
- **触发条件**：出现"存在但不可见的节点数 ≫ 可见节点数"的负载（≥10³–10⁴ 节点、可见 <1%），
  或那份负载的 profile 显示收集侧成为主项。
