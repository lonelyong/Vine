...
| **R5** | appfw 的插件注册表在 LSan 下**每次都报**（后端门禁里唯一的报告） | `vine::runtime::DynamicLibraryLoader` 单例（`.cpp:76` 构造、`:190` load）→ `PluginManager::loadAll`：跑完 `test_vsg` 报 808 B / 16 次分配，栈里**零 vsg 帧** | 二选一：让 loader 在退出时释放，或把它声明为“进程生命周期保留”并进 `scripts/asan_leaks.supp` —— **但那份文件自己的规则写着“框架自己的分配不许抑制”** ⇒ 更可能要改代码 | 属 appfw，不在后端范围；现在门禁的做法是**报出但不判**（`VINE_ASAN_LEAK_SCOPE`），所以它不会再遮住后端结论 | 待办（appfw） |
