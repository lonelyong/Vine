...
| `shaders/vine_forward.{vert,frag}` + `detail::buildVineShaderSet` | **自写前向着色**（替代 vsg 内建 phong 的 P0 第一步）：属性 0/1/2(色,define)/8(uv,define)、set0 的 material(b0) / diffuseMap(b1) / **vine_lights(b2, 每视图 UBO)** + push `pc` 0..128（vsg 矩阵栈填）。**尚未接线**：没有 pass 用它，默认路径不变（ABI/门禁见该文档 §11） |
