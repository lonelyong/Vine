...
> ⇒ **判词**：这笔差**不是我们的逻辑或簿记**（工作逐行相同、计数相同、帧数无关、无热点），而是**同样工作量下软件光栅器的一次性开销**变了；为什么会变，本机工具查不到（"改地址/分配时序影响 llvmpipe 内部映射"只是剩余猜想，且被 ⑤ 削弱）。**未做**：release（-O2）复测——那才是"交付物是否带这笔钱"的判据，代价是一次全量 reconfigure+build。上面那句"更像代码布局效应"的旧猜测经 ⑤ 逐条检验后**不成立**，已删。
