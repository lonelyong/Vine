...
> - **机制查清了（2026-09-16 深夜，装好 valgrind/gdb 之后）：那 0.6 s 是 glslang 在重编内建符号表 —— 而且它不在驱动里，在我们自己的编译路径里。** `valgrind --tool=callgrind` 两边各跑一次 6 帧（A **16,597,898,524** 条指令 / B **19,514,662,986**，**+2.92 G，+17.6%**），逐函数相减：多出来的**几乎全是 glslang 在解析** —— `yyparse` 1.61 G → 2.60 G、`TPpContext::tStringInput::getch` 1.12 G → 1.72 G（**+54% 的输入字符**）、`TPoolAllocator::allocate`、`TType::deepCopy`、`TScanContext::tokenizeIdentifier` …… 而**着色器源码与编译份数完全相同**（`combineSourceAndDefines` 182 + 21200 次两边一致、`TShader` 对象 279 个、`createShaderModule` 208 次、`ShaderCompiler::compile` 421 次、日志逐行一致，连那条 `Warning: GLSL source failed to parse.` 也一致）。差别在**glslang 的内建符号表被重建了更多次**：`InitializeSymbolTable` 发起的解析 **534 → 714**、`TSymbolTable::insert` **199 k → 339 k（+70%）**、`SetupBuiltinSymbolTable` 的**每次**代价 27 M → 46 M 条指令。
> - **为什么会被重建：vsg 用引用计数管着 glslang 的进程生命周期**（`vsg/src/vsg/utils/ShaderCompiler.cpp:51/59` 的 `s_initializeProcess`/`s_finalizeProcess`：计数归零才 `glslang::InitializeProcess()`，减到零就 `glslang::FinalizeProcess()`）—— **`FinalizeProcess()` 会把 glslang 的内建符号表缓存丢掉**，于是下一次 `TShader::parse` 必须**从头解析内建符号源**（每次约 50 ms）。callgrind 数出来：**A 17 次、B 29 次** `InitializeProcess`/`FinalizeProcess`（+12 个 epoch ≈ +180 次内建解析 ≈ +2 G 指令 ≈ 那 0.6 s）。ⓘ 与驱动无关：llvmpipe 的光栅线程、JIT 次数（`exec_mprotect` 219 对 219）两边一样，多出来的全是**主线程上的 glslang CPU**。
> ⇒ **判词（最终）**：这笔钱是 **glslang 编译**，是我们编译路径上一笔**可省的启动成本** —— 不是"光栅器的开销"，也不是"与本次逻辑无关"；glslang 是纯 CPU 的，所以**真机同样要付**。上面那些多出来的 16 MB 设备内存分配是**同一批事件里的另一项观察**（多编译/新建出来的东西要分配），两者因果未证，不要当成一个原因。
> - **受益面：给了个自我否定的答案（2026-09-16 深夜，H8）**。同一个探针量两处（gdb 断在 `glslang::InitializeProcess/FinalizeProcess`）：**自检（6 帧）31 个 epoch，而 GUI 应用跑 25 秒只有 `init=2 finalize=1`** ⇒ **交付的应用根本不付这笔钱**（它只建一次会话，`ShaderCompiler` 的引用计数不归零，glslang 的内建符号表就不会被丢）。那 +0.6 s 是**自检故意反复建/拆会话**这个**工作负载**的产物（A 17 / B 29 / HEAD 31），**不是这次改动带给产品的回归** ⇒ **不做"常驻 `ShaderCompiler`"那个改动**（它只对"不断重建整会话"的负载有用）。登记为：**若将来某个负载真的反复重建会话，每个 epoch 约 50 ms**。
> - **一条想当然的因果，经查不成立**：T16 把释放从三处批量折成了每槽的 `VsgCompileManager::forget()`（HEAD 实测 **93** 次），看起来很像 epoch 变多的原因；但实测**前 7 次 finalize 之前一次 forget 都没有**，forget 是成片出现的（另有一股 79 次）⇒ "forget 直接导致 finalize 次数上升"**不成立**。epoch 数由工作负载的会话/相位结构决定，**具体落点未查**——而应用侧受益为零，不值得继续挖。
...
**新增一类窗口**（`include/vine/vsg/VsgHostWindow.hpp` + `src/VsgHostWindow.cpp`）：`detail::VsgHostWindow : public ::vsg::Inherit<::vsg::Window, VsgHostWindow>`，实现 vsg 那两个纯虚（`_initSurface()` **和** `instanceExtensionSurfaceName()`），X11 分支给出 `VK_KHR_XCB_SURFACE_EXTENSION_NAME`、Win32 分支 `VK_KHR_WIN32_SURFACE_EXTENSION_NAME`。**两个平台都已实测（2026-09-17：Windows 11 + RTX 4060 跑通自检相位与 app 门禁，四条判据见 `.ai/memory/graphics-perf-backlog.md` 的 H1）**。
...
- **绝不销毁**：析构 `clear()` 后把 `_window` 置空，**基类析构因此不会** `xcb_destroy_window()`／`::DestroyWindow()`（Win32 那条路还会顺手 `UnregisterClass(GetClassName(hwnd))`——对 Qt 的类是灾难）。这就是这一层存在的唯一理由。**还有一半（2026-09-17 在 Windows 上实测发现）**：vsg 的 `Win32_Window` 在**采纳**分支也会调 `_initDrop()`，把 OLE 拖放目标注册到**宿主窗口**上，而它的撤销（`_shutdownDrop()`）只在基类析构里；那时本类已把句柄置空（不置空就会被 `DestroyWindow`），**搬移后句柄更不是注册时那个** ⇒ 宿主窗口留下一个指向已 `Release()` 的 target 的注册（拖上去碰已释放内存），且此后该窗口 `RegisterDragDrop` 永远失败（实测：修复前同一窗口第二次被采纳时报 `Warning: Win32_Window::_initDrop() RegisterDragDrop failed`，一轮 2 次；构造末尾调 `withdrawHostWindowState()` 后 0 次）。**为什么在构造里撤而不是析构里**：只有刚采纳的那一刻，“手上的句柄”与“被注册的窗口”一定一致。X11 侧对应物只是写窗口自己的 `XdndAware` 属性（没有我们持有的对象），所以那个函数在 X11 是空实现。
