...
| D38 | **借来的深度会"冻结"且借用的记录顺序缺边**：`shareDepth` 的目标把源的深度图像**烧进自己的帧缓冲**，但只在自己重建时重新校验——`render()` 的重建谓词只看自己的尺寸/策略，看不见**源被重建**（同尺寸重建：混合深度策略收敛、尺寸变化、模板改动都会重建源）。源一重建就换掉深度图像，借用方的帧缓冲继续测试**没人再写的旧图**：借来的深度静默冻结（旧图还被 frame buffer 握着，白留一份显存）。另一半是记录顺序：`reconcileOffscreenOrder` 的依赖边只来自 PiP/全屏 program 的**采样**属性，深度借用**不是边** —— 源在借用方之后重建时它被追加到 `children` 末尾（`command_graph->children.push_back`）→ 借用方的 render graph 排在源**之前**，整帧用上一帧的深度（布局相同、无 VUID、纯静默 1 帧滞后）。**已修（2026-09-11，设计 §25）**：`Target::depth_source_view` 记住烧进帧缓冲的那张源视图，`render()` 里 `borrow_stale`（源视图变了 → 重建 → 重跑借用校验，含墓碑守卫防止诊断循环）+ `reconcileOffscreenOrder` 把 `depth_source` 也当一条依赖边。守卫：selftest `runDepthShareOrderPhase`（借用方用 `TestOnly` 只测不写；源在第 2 帧因混合策略**同尺寸重建**、第 3 帧加入更近的面 → 从第 3 帧起必须帧帧被拒；只滞后 1 帧 = 缺边，一直不拒 = 旧图），反证：停掉 `borrow_stale` → 断言红（`AAAAAA`）；停掉依赖边在当前实现下**不可观测**（借用方的 `borrow_stale` 重建会把图重新追加到末尾，等价修复顺序），该边作为不变量保留 | `Target` / `VsgRendererTargets::buildOffscreenTarget` / `VsgRenderer::render` / `reconcileOffscreenOrder` | 🟢 |

| D39 | **目标描述在构建后改变被静默忽略**：`buildOffscreenTarget` 把目标的附件数量/格式、深度格式、深度提升标志烧进图像 + render pass + framebuffer，但 `render()` 的重建谓词只逐项列了尺寸/深度策略/借用待办/借用过期 —— **没被列到的属性一律静默失效**。症状一：中途 `attachColor()` → 帧缓冲仍是旧附件数 → 新附件**永远不存在**，`readColorBuffer(t,1)` 报 "out of range"（听着像"不支持"，其实是后端承认丢了请求）。症状二：中途 `setDepthPromotion(true)` → **变更帧重建 0 次**（反证实测），深度从未被提升；更糟的是 §23 的借用校验读的是**构建时烧下的** `depth_sampleable`，于是继续放行借用 → 借用方把一个源 pass 已按采样布局收尾的深度当附件挂上（§23 拦下的错误从后门静默回来）。**已修（2026-09-11，设计 §26）**：`Target::BuildKey`（颜色附件数 + 各格式 + 是否有深度 + 深度格式 + 提升标志）构建末尾记录、重建重置块清空、重建谓词整把比较；尺寸/深度策略仍是独立项（借用校验要读已建尺寸、`releaseRenderTarget` 靠清零逼重建；`depth_load` 来自本帧 pass 请求）。守卫：selftest `runTargetDescriptionChangePhase` 两段（加附件 → 附件 1 必须读回透明黑且恰好重建 1 次；开提升 → 源 + 借用方恰好重建 2 次、借用被拒报 1 次、远面必须又能画出来），反证：停掉指纹项 → 4 条断言同时红（`never existed` / `kept borrowing it` / `not reported` / `0 time(s)`） | `Target::BuildKey` / `VsgRenderer::render` / `buildOffscreenTarget` | 🟢 |
