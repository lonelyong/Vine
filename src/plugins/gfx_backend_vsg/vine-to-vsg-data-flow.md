...
| ID | 缺陷 | 位置 | 严重度 |
|---|---|---|---|
| D20 | 验证原先只有“无 VUID”，**没有任何像素断言**。**已补（2026-09-11，设计 §15）**：`VsgRenderer::readColorBuffer` 落地（RGBA8，离屏，blit 到线性图 + 映射回读；float 附件诚实报不支持），selftest 新增像素阶段（中心像素=被光照的红四边形、角像素=清屏色、float 附件返回 false），harness 把 `[selftest] FAIL` 当硬失败并打印实际使用的设备。**已铺开（设计 §17）**：PiP blit（子矩形内外 + 面积精确）、deferred 全屏程序（整张填满）、深度顺序（开测试时蓝色像素 0 个）、深度模式权威（Disabled 时中心变蓝）、MRT 附件 0；harness 打印证据并要求 ≥5 行像素断言。**仍需**：真机 GPU 冒烟（本机只有 llvmpipe/lavapipe，无 GPU 驱动） | 验证 | 🟡 |
| D31 | **MRT 附件 ≥ 1 一律清成透明黑**（附件 0 才拿 pass 的 `clear()` 颜色）—— 有意为之（`VsgRendererTargets`："empty regions stay black until a fragment writes them"），但此前没有任何东西验证过；像素探针第一次跑即量到附件 1 整张 (0,0,0)。对采样额外附件的消费者（deferred 读 G-buffer 法线）可见：未覆盖像素拿到零法线而非清屏色。**待决策**：写进消费者契约，或统一清成同色（后者需先审 deferred 输出） | `VsgRendererTargets` 离屏构建 | 🟡 |
