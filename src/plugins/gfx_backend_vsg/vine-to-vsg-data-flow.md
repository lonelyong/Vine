...
| D36 | **深度借用不做可用性校验**：`shareDepth` 把源的深度图像当本目标的深度附件用，但只检查了"源本帧没渲染过"。另两种不可用情形全静默：**尺寸不同** → 非法帧缓冲（`VUID-VkFramebufferCreateInfo-pAttachments-00880`）+ 未定义渲染（llvmpipe 上看起来正常）；**源把深度提升成可采样** → 源的深度在 `SHADER_READ_ONLY_OPTIMAL` 而 render pass 声明的是附件布局，**每帧** `VUID-VkImageMemoryBarrier-oldLayout-01197` 且**什么都没画**。**已修（2026-09-11，设计 §23）**：构建时三向校验（源本帧有深度图 / **尺寸相同** / **未提升**，后者用构建期标志 `Target::depth_sampleable`），失败则 `ContentSkipped` **报一次** + 墓碑 `unusable_depth_source` + **回落自有深度**。契约：借深度要求源同尺寸且 `setDepthPromotion(false)` | `VsgRendererTargets::buildOffscreenTarget` / `Target` | 🟢 |
