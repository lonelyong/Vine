...
TEST(GeometryTest, TheTexcoordSlotSpellsTwoWidthsAtOneLocation)
{
    // The setters state a WIDTH and nothing else: three scalars per vertex here (what a cube map is sampled
    ...
    auto segment = intrusive_ptr<Geometry>(new Geometry());
    segment->setTexcoords3(directions, 0u, 6u);
    ...
    EXPECT_EQ(whole->texcoordComponents(), 3u);
    ...
    EXPECT_EQ(uv_geometry->texcoordComponents(), 2u);

    // An empty slot has no width at all, which is what "no channel" has to look like to a reader that asks.
    auto empty = intrusive_ptr<Geometry>(new Geometry());
    EXPECT_FALSE(empty->hasTexcoords());
    EXPECT_EQ(empty->texcoordComponents(), 0u);
}
