...
using vine::app_shell::fitPreviewRect;
...
TEST(PreviewFitTest, AWiderSourceIsLetterboxedVertically)
{
    ...
    const vine::graphics::Viewport rect = fitPreviewRect(32, 16, 3, 4, 10, 10);
    EXPECT_EQ(rect.x, 3);
...
TEST(PreviewFitTest, ATallerSourceIsPillarboxed)
{
    ...
    const vine::graphics::Viewport rect = fitPreviewRect(16, 32, 3, 4, 10, 10);
...
{
    const vine::graphics::Viewport rect = fitPreviewRect(320, 200, 8, 8, 160, 100);
    EXPECT_EQ(rect.x, 8);
    EXPECT_EQ(rect.y, 8);
...
TEST(PreviewFitTest, ADegenerateSourceAnswersTheSlotItself)
{
    ...
    for (const vine::graphics::Viewport& rect : { fitPreviewRect(0, 200, 8, 8, 160, 100),
...
TEST(PreviewFitTest, ADegenerateSlotAnswersTheSlotAsGiven)
{
    ...
    const vine::graphics::Viewport no_width = fitPreviewRect(320, 200, 8, 8, 0, 100);
    EXPECT_EQ(no_width.x, 8);
    EXPECT_EQ(no_width.y, 8);
    EXPECT_EQ(no_width.width, 0);
    EXPECT_EQ(no_width.height, 100);

    const vine::graphics::Viewport no_height = fitPreviewRect(320, 200, 8, 8, 160, 0);
...
TEST(PreviewFitTest, AVeryThinSourceStillGetsAPixel)
{
    ...
    const vine::graphics::Viewport rect = fitPreviewRect(100, 1, 0, 0, 4, 4);
